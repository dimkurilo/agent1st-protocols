...
Пять правил и метапромпт-шаблон. GLM 5.2 - не DeepSeek, не Qwen, не GPT. Она думает иначе, и промпты под неё нужно писать иначе.

> Актуально для протокола `agent1st_v10-glm.md` (533 строки). Гайд обновлён 2026-06-18.
...
## Правило 1: Используй структуру Goal, Context, Constraints, Done
...
|---------|---------------------------|------------------|
| **Goal** | Активирует long-horizon planning. Чёткая цель помогает модели строить траекторию | `max` |
...
- Числа, версии, даты: пиши явно, например `v2.3`, `2026-06-17`
...
```
### Context
- Файл: ~/.config/opencode/agents/agent1st_v10-glm.md
...
|-----------|-------------------|
| «Сожми траекторию, удали лишние шаги» | Модель не управляет историей сообщений - это делает обвязка запуска (harness) |
...
- «Составь краткое summary выполненного - я передам его harness для `/compact`»
- «Сигнализируй harness, что контекст перегружен - выведи `## STITCH COMPRESSION`»
...
GLM 5.2 обучена самостоятельно исправлять ошибки. После неудачного вызова нужно перечитать source, найти root cause, внести минимальное исправление и перепроверить. Не проси «пожалуйста, проверь себя» - это обученное поведение.
...
GLM 5.2 под `max` может stall на негативных constraints, если в них есть только «не делай X» без положительного действия. Связывай каждый `don't` с конкретным `do`: `Don't narrate`; `Keep output to results and evidence`.

---

## Правило 5: Метапромпт-шаблон

**Как использовать template:**
1. Опиши задачу свободным текстом (что хочешь сделать)
2. Вставь метапромпт-template ниже
3. Попроси агента: «погрузись в контекст и составь готовый промпт по template»
4. Получи заполненный промпт и используй его в новой сессии
...
**Как агент заполняет template (context-loading правила):**
- Context: указать файлы с полными путями, потому что DSA ищет точные токены
- Если результатов три или больше, отметить Workflow Mode: Plan, Execute, Audit
- Для non-trivial задач: path-is-clear gate, где агент показывает plan в 1-3 sentences и ждёт подтверждения
- Body changes к agent protocols: mini-test required, harness-bench, 8 tasks by 2 passes
...
### Context
- Файлы:
  - ~/.config/opencode/agents/agent1st_v32-deepseek.md - аудируемый агент (920 строк)
  - evolution/deepseek/V25-V29-TRANSITION.md - design principles history
  - research/analysis/deepseek_v4.md - paper extract
...
- Стиль: VS-CoT Pattern C - 5-7 гипотез с probability and impact
...
- Формат результата: распределение гипотез с probability-impact ranking и MEMORY.md update
```
...
```
Структура промпта для GLM 5.2: Goal, Context, Constraints, Done
...
Писать:
  ...
  - точные версии и числа (v10, 2026-06-18, 1024)
  - критерий готовности: команда и ожидаемый результат
  - что НЕ трогать (Constraints)
  - первые предложения - самые важные (IndexCache)

API (не промпт):
  - reasoning_effort: max или high, потому что low/medium всё равно маппятся на high
  - do_sample: false для детерминированного кода
  - thinking.type: enabled по умолчанию
...
Метапромпт (создание промптов):
  Опиши задачу, вставь template, попроси «погрузись в контекст и заполни».
  Полученный промпт используй в новой сессии.
```
...
| Фича v10 | Где | Зачем |
|----------|-----|-------|
| Path-is-clear gate | §1 | Non-trivial task: подтверди plan (1-3 sentences), жди `go` |
...
| Self-Harness meta-loop | §4.4 | After 3+ failures: mine patterns, then write concrete rules |
| Workflow Mode | §3 | Three or more deliverables: Plan, Execute, Audit |
| Subagent selection | §7.1 | Semantic/local: GLM-native (`auditor-glm`, `explore-glm`); web/mechanical: DeepSeek (`general`, `verifier`, `review`) |
| STITCH trajectory | §5 | Signal harness для `/compact`; модель не удаляет историю, harness делает |
| Preserved Thinking | §2.1 | reasoning_content preserved - reuse, не re-derive |

---

*Гайд обновлён под `agent1st_v10-glm.md` (533 строки). Предыдущая версия ссылалась на v8 (402 строки) - устарела. Метапромпт-секция добавлена 2026-06-18 под пользовательский рабочий процесс.*
