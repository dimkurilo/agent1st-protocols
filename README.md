...
Runtime для зрения (vision) - отдельный репозиторий: [vision-bridge-opencode](https://github.com/dimkurilo/vision-bridge-opencode). Без него text-only агент просто работает как text agent.
...
**[vision-bridge-opencode](https://github.com/dimkurilo/vision-bridge-opencode)** - дать text-only агенту OpenCode зрение без смены модели на vision-модель.
